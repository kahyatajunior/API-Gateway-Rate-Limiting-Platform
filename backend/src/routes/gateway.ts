...
async function recordUsage(
  ...
  await client.query(
    `INSERT INTO api_usage(api_key, path, method, status, latency_ms, blocked_reason)
     VALUES($1, $2, $3, $4, $5, $6)`,
    [apiKey, path, method, status, latencyMs, blockedReason]
  );
}
...
router.all('/*', async (req, res) => {
  ...
  const gatewayPath = `/${(req.params as { 0?: string })[0] ?? ''}`;
