...
router.post('/generate', async (req, res) => {
    ...
    const client = await connectDb();
    await client.query('INSERT INTO api_keys(api_key, owner, quota, target_url) VALUES($1, $2, $3, $4)', [apiKey, owner, quota, targetUrl]);
...
router.patch('/keys/:apiKey', async (req, res) => {
    ...
    const client = await connectDb();
    const result = await client.query(`UPDATE api_keys
     SET quota = COALESCE($2, quota),
         target_url = COALESCE($3, target_url),
         revoked = COALESCE($4, revoked)
     WHERE api_key = $1
     RETURNING *`, [req.params.apiKey, quota, targetUrl, req.body.revoked]);
    if (result.rowCount === 0) {
...
router.delete('/keys/:apiKey', async (req, res) => {
    const client = await connectDb();
    const result = await client.query('UPDATE api_keys SET revoked = TRUE WHERE api_key = $1 RETURNING *', [req.params.apiKey]);
    if (result.rowCount === 0) {
        return res.status(404).json({ error: 'API key not found' });
    }
    res.json({ key: result.rows[0] });
